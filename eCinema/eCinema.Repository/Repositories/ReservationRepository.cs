...
namespace eCinema.Repository.Repositories
{
    public class ReservationRepository : BaseRepository<Reservation, int, ReservationSearchObject>, IReservationRepository
    {
        ...
        {
            return await dbSet.Include(x => x.Show).ThenInclude(x => x.Movie).Include(x => x.Seats).ThenInclude(x => x.Seat).Where(x => x.UserId == userID).OrderByDescending(x => x.ID).ToListAsync();
        }
